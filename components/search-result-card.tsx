...
export function SearchResultCard({ id, name, image, price, rating, location, distance, priceUnit, categoryName }: SearchResultCardProps) {
  const { t } = useLanguage()

  // Resolve price unit display
  const unitDisplay = priceUnit
    ? t(`unit.${priceUnit.toLowerCase()}`)
    : t("unit.hour")
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center gap-3 lg:gap-4 py-2 lg:py-3 border-t border-b border-border/50">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs lg:text-sm font-medium text-muted-foreground">₹{price}/{unitDisplay}</span>
          </div>
          {categoryName && (
            <>
              <div className="w-px h-3 lg:h-4 bg-border"></div>
              <div className="flex items-center gap-1.5">
                <span className="material-symbols-outlined text-muted text-[18px]">category</span>
                <span className="text-xs lg:text-sm font-medium text-muted-foreground">{categoryName}</span>
              </div>
            </>
          )}
        </div>
