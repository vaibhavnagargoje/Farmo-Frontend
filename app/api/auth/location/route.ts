...
export async function POST(request: NextRequest) {
  try {
    ...
    // Update location via Django API
    const response = await fetchWithAuth(API_ENDPOINTS.UPDATE_LOCATION, token, {
      method: "POST",
      body: JSON.stringify({
...
export async function GET() {
  try {
    ...
    // Get profile via Django API
    const response = await fetchWithAuth(API_ENDPOINTS.USER_PROFILE, token, {
      method: "GET",
    })
