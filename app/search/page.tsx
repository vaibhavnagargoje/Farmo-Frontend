...
function SearchContent() {
  ...
  useEffect(() => {
    const fetchResults = async (q: string) => {
      setLoading(true)
      try {
        const res = await fetchPublic(API_ENDPOINTS.SEARCH(q))
        if(res.ok) {
      ...
      <div className="flex justify-between items-center mb-6 max-lg:px-6 max-lg:mt-2">
        <div>
          ...
          <p className="text-sm text-muted mt-1">
             {loading ? "Searching..." : `${results?.total_services || 0} equipment found for "${initialQuery}"`}
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 max-lg:px-6">
          {results.services.map((srv) => (
            <SearchResultCard
               ...
               id={srv.id.toString()}
               name={srv.title}
               image={srv.thumbnail || "/placeholder.svg"}
        ...
        <div className="flex flex-col items-center justify-center p-12 text-center bg-card border border-border rounded-2xl max-lg:mx-6">
           ...
           <p className="text-muted-foreground text-sm max-w-sm">
             We couldn't find any equipment matching "{initialQuery}". Try adjusting your search term or using broader keywords.
           </p>
        </div>
      )}
    </div>
